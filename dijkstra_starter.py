...
def visit_next(visited, unvisited, distances):
    ...
    neighbour_list = distances[visited[len(visited)-1][0]] #1. access the last item in visited (will be the city you just moved (the closest one)) - you need -1 for this; 2. for this city, retrieve all of its neighbours from distance dict
    dist_closest_city_destination = visited[len(visited)-1][1]

    tuple_to_list = [list(x) for x in neighbour_list] #problem is that i'm trying to change tuple; need to convert to list then change back to tuple

    for neighbour_city in tuple_to_list: #looping through each neighbour and considering the 3 cases
        if neighbour_city[0] in [x[0] for x in visited]: #if we've already visited that city, we've already found the shortest distance of that city to the destination city, so dont need to add to unvisited and/or update distances
            pass

        elif neighbour_city[0] not in [x[0] for x in unvisited]: #if neighbour not in unvisited, append (make sure to update the distance of each neighbour to the destination city)
            neighbour_city[1] += dist_closest_city_destination #adjust distances of new neighbours for distance of closest city to destination city
            ...
            for i in range(len(unvisited)):
                if unvisited[i][0] == neighbour_city[0] and unvisited[i][1] > neighbour_city[1] + dist_closest_city_destination:
...
#/Library/Frameworks/Python.framework/Versions/3.4/bin/python3.4 "/Users/nicolasahar/Desktop/ECs/2015-2016/Computing for Medicine (Apr 2016)/Assignments/Assignment 2 (due July 29)/Starter Code/e5_tester.py"
#Test 1 passed
#Test 2 passed
#Test 3 passed
#Test 4 passed
...
### Extra Code (Comment Out) - E5 Analogue Question (PCRS) ###

#In this problem, we will be simulating a patient queue. For each patient, we have a name, and a priority code.
# The patients are to be treated according to their priority -- higher priority first. The function visit_next_patient(visited, unvisited)
# is supplied. The function finds the patient with the highest priority in the list unvisited, and moves their name to the list visited.
# If visit_next_patient() is called repeatedly, visited will become a list of patients sorted by their priority, and unvisited will become empty.

#Write the function visit_all_patients(), which should call visit_next_patient repeatedly while it makes sense to, and returns a list of
# patients sorted by priority. Use visit_next_patient rather than sorted() or sort() to accomplish this.
...
def visit_next_patient(unvisited, visited):
    ...
    for i in range(len(unvisited)):
        if unvisited[i][1] > highest_priority_val:
            highest_priority_val = unvisited[i][1]
            highest_priority_ind = i

    visited.append(unvisited[highest_priority_ind][0])
    unvisited.pop(highest_priority_ind)
    #alt way to splice: unvisited[highest_priority_ind:highest_priority_ind + 1] = []

    # return visited, unvisited (for testing purposes)

## Test cases for visit_next_patient
#unvisited = [("Bob", 5), ("Tom", 6), ("Tim", 4)]
#visited = ["Alice"]
#print(visit_next_patient(unvisited, visited))

def visit_all_patients(patient_list):
    #Return a list of the names of the patients, from highest priority to lowest, without using sorted() or sort(). Instead, use calling visit_next_patient
   #repeatedly.

   #Arguments:
   #   patient_list -- a list of tuples, with each tuple containing the name of a patient (str) and their priority (int)
   ...
   #["Tom", "Bob", "Tim"]
    ...
    for i in range(len(patient_list)):
        visit_next_patient(patient_list, visited)

    return visited

## Test Case for visit_all_patients
#patient_list = [("Bob", 5), ("Tom", 6), ("Tim", 4)]
#print(visit_all_patients(patient_list))
